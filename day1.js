...
for (const line of lines) {
    ...
    for (const char of line) {
        if (!isNaN(char)) {
            calibrationSegments.push(char)
        }
    }
    const wantedSegments = [calibrationSegments.slice(0)[0], calibrationSegments.slice(-1)[0]];
    const calibrationValue = wantedSegments.join('')
    calibrationValues.push(calibrationValue);
}

const final = calibrationValues.filter(n => n);

const finall = final.map(n => Number(n));

const finalll = finall.reduce((a, b) => a + b, 0);

console.log("part 1: " + finalll);

// part 2
const digits = ["zerone", "twone", "oneight", "threeight", "fiveight", "sevenine", "eighthree", "eightwo", "nineight", '1', "2", "3", "4", "5", "6", "7", "8", "9" ,"one", "two", "three", "four", "five", "six", "seven", "eight", "nine"];
const matchy = new RegExp(digits.join("|"), 'gi');
const results = [];

for (const line of lines) {
    const digs = line.match(matchy);
    let result = "";
    let first = digs[0];
    if (isNaN(first)) {
        switch (first) {
