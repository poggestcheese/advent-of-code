...
for (const game of games) {
    const gameID = Number(game.substring(0, game.indexOf(": ")).replace("Game ", ""))
    const content = game.replace(`Game ${gameID}:`, "");
    const handy = content.split(";")
    const objectt = {
        gameID: gameID,
        hands: [],
    };
    for (const hand of handy) {
        const blueIndex = hand.search("blue")
        const blues = hand[blueIndex - 3] + hand[blueIndex - 2];
        const redIndex = hand.search("red")
        const reds = hand[redIndex - 3] + hand[redIndex - 2];
        const greenIndex = hand.search("green")
        const greens = hand[greenIndex - 3] + hand[greenIndex - 2];
        const objecttt = {
            red: reds,
            green: greens,
            blue: blues
        }
        objectt.hands.push(objecttt);
    }
    gamesObj.push(objectt);
}
...
for (const gameObj of gamesObj) {
    stopp = 0;
    const gameID = gameObj.gameID;
    for (const hand of gameObj.hands) {
        if (hand.red > criteria.red) {
            stopp = 1
            break;
        }
        else if (hand.blue > criteria.blue) {
            stopp = 1
            break;
        }
        else if (hand.green > criteria.green) {
            stopp = 1
            break;
        }
        else {
...
for (const gameObj of gamesObj) {
    ...
    let sBlue = 0;
    for (const hand of gameObj.hands) {
        if (Number(hand.red) > sRed) {
            sRed = Number(hand.red);
        }
        if (Number(hand.green) > sGreen) {
            sGreen = Number(hand.green);
        }
        if (Number(hand.blue) > sBlue) {
            sBlue = Number(hand.blue);
        }
        else {
            // Any NaNs is fine; means there was no match for a colour in that specific hand
            // Any undefined is bad
            // Debugging purposes only
            // console.log(gameObj.gameID + ` | ${hand.red}, ${hand.blue}, ${hand.green}`);
        }
    }
    const smallest = {
...
for (const game of smallests) {
    const power = Number(game.red) * Number(game.green) * Number(game.blue);
    powers.push(power);
}
